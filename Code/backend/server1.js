...
app.get("/api/condition-images/:item_id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.execute(
      "SELECT id, filename, created_at FROM conditionimages WHERE item_id = ? ORDER BY created_at ASC",
      [item_id]
    );

    // rows will be an empty array if no images exist
...
app.post("/api/condition-images/:item_id", authenticateToken, uploadConditionImage.single("image"), async (req, res) => {
  const item_id = req.params.item_id;
  ...
  try {
    ...
    const [result] = await db.execute(
      "INSERT INTO conditionimages (item_id, filename, created_at) VALUES (?, ?, NOW())",
      [item_id, filename]
    );
